...
export default function OrderCard({ order }: OrderCardProps) {
  ...
  const handleContinuePayment = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    <div className="border border-border p-4 md:p-6 hover:border-foreground transition-colors bg-background">
      ...
      <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
        ...
        <div className="w-full md:w-auto flex flex-col md:items-end gap-3 border-t md:border-t-0 border-border pt-4 md:pt-0 mt-2 md:mt-0">
          <div>
            <p className="font-body text-xs text-muted-foreground md:text-right mb-1">
              Total Belanja
            </p>
          ...
          <div className="w-full md:w-auto flex flex-col sm:flex-row gap-2">
            {canContinuePayment && (
              <button
                ...
                className="w-full sm:w-auto text-center px-6 py-2 bg-foreground text-background font-body text-[10px] tracking-[0.2em] uppercase border border-foreground hover:bg-background hover:text-foreground transition-colors duration-300"
              >
            ...
            <Link
              ...
              className="w-full sm:w-auto text-center px-6 py-2 bg-background text-foreground font-body text-[10px] tracking-[0.2em] uppercase border border-foreground hover:bg-foreground hover:text-background transition-colors duration-300"
            >
              Lihat Detail
            </Link>
          </div>
